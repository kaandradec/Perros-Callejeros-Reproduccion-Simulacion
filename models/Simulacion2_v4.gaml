...
//	shape_file building_file <- shape_file("../maps/buildings.shp"); //fixed not whole mayondon
	...
	int numeros_perros_macho <- 25; // Número inicial de perros macho 65.5% machos
	int numeros_perro_hembra <- 20; // Número inicial de perros hembra
	...
	int capacidad_jaulas <- 12;   // Número de jaulas en el remolque
...
species perros_macho skills: [moving] {
   ...
   // Reflex para buscar comida cuando está hambriento
    ...
    action sterilize {
        esterilizado <- true;
        // Increment global counter ONLY when a dog is sterilized for the *first* time
        // Check if it was not already sterilized before setting it
        // if not esterilizado { // This check is done by the carro before calling
             total_perros_esterilizados <- total_perros_esterilizados + 1;
        // }
        write "Perro Macho: Esterilizado";
    }
...
//    }
	...
	//	is_working_time <- true;
		int total_perros <- count (perros_macho, true)  + count (perros_hembra, true);
...
//	reflex update_working status {
//		int total_perros <- perros_macho count + perros_hembra count;
//		
//	}
  ...
  // Reflex of movement: deambulate randomly by the network when working
    ...
    // Execute movement towards the target
    reflex execute_move when: target != nil {
        do goto target: target on: road_network;
        if location = target { // If arrived at the target
            target <- nil; // Clear the target
            // write "Carro: Arrived at target."; // Debugging
        }
    }
	...
	reflex capturar when: is_working_time {
	    ask perros_macho at_distance 15 { // Radio reducido
	        if (not esterilizado and myself.jaulas_ocupadas < capacidad_jaulas) {
	            do sterilize; // Call the dog's sterilize action
                myself.jaulas_ocupadas <- myself.jaulas_ocupadas + 1; // Increment car's temporary count for the current trip/day
                // Global total_perros_esterilizados is incremented inside the dog's sterilize action
                write "Carro: Esterilizado un perro/a en " + location; // Message upon successful sterilization
	        }
	    }
	}
...
//	}
 ...
 // Reflex to reset jaulas_ocupadas when the car stops working
    reflex reset_jaulas when: not is_working_time and jaulas_ocupadas > 0 {
		 write "Carro: Parando de trabajar. Vacío jaulas (" + jaulas_ocupadas + ").";
		 jaulas_ocupadas <- 0; // Reset the car's temporary count
	}
	...
	output {
		display map_2D type: 2d {
				...
				display pie1 refresh: every(10#cycles)  type: 2d {
					...
					// Pie chart for Sterilization Status
           			 chart "Perros Esterilizados vs No Esterilizados" type: pie style: exploded size: {1, 0.5} position: {1, 0.5} { // Positioned next to the first pie
              		  data "esterilizados" value: (perros_macho count (each.esterilizado = true)) + (perros_hembra count (each.esterilizado = true)) color: #black;
              		  data "no esterilizados" value: (perros_macho count (each.esterilizado = false)) + (perros_hembra count (each.esterilizado = false)) color: #white;
            		}
