...
//	shape_file building_file <- shape_file("../maps/plaza_const.shp"); //fixed not whole mayondon
	...
	int nb_cats_male <- 25;
	int nb_cats_female <- 25;
	float agent_speed <- 4 #km / #h;
	float proba_breed <- 0.02;
	float step <- 1440 #mn;
	int number_of_food_source <- 5;
	int when_hungry_get_food <- 300;
	int die_when_hungry <- 700;
	int max_offspring <- 6; //number of kittens can be reproduced by female cats
	float accidental_death_proba <- 0.0001;
